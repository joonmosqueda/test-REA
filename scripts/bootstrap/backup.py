...
class S3Backups(object):
    ...
    def backups(self):
        ...
        for k in self.__list_backups():
            if self.KEY_SUFFIX not in k:
                continue
            backups.append(k.replace("%s/" % self.__bucket_prefix,
                                     '').replace(self.KEY_SUFFIX, ''))
        return backups
    ...
    def backup(self, file_path, backup_name):
        k = Key(self.__b)
        k.key = os.path.join(self.__bucket_prefix,
                             backup_name + self.KEY_SUFFIX)
    ...
    def delete(self, backup_name):
        self.__b.delete_key(os.path.join(
            self.__bucket_prefix, backup_name + self.KEY_SUFFIX))
        return os.path.join(self.__bucket_prefix, backup_name)
...
@click.option('--bucket-region', type=click.STRING, default='ap-southeast-2', envvar='JENKINS_BACKUP_BUCKET_REGION', help='S3 bucket region : defaults to "ap-southeast-2"')
...
@cli.command()
@click.pass_context
@click.option('--jenkins-home', type=click.STRING, default='/data/jenkins_home', help='Jenkins home directory : defaults to "/var/lib/jenkins"')
...
@click.option('--exclude-plugins/--include-plugins', default=True, help=' do not backup plugins')
...
@click.option('--exclude-builds/--include-builds', default=False, help='Exclude job builds directories from the backup : defaults to true')
@click.option('--exclude-build-definitions/--include-build-definitions', default=True, help='Exclude config.xml files from the jobs : defaults to true')
...
@click.option('--exclude-logs/--include-logs', default=False, help='Exclude logs from the backup : defaults to true')
@click.option('--exclude-init-jobs/--include-init-jobs', default=True, help='Exclude init jobs used in bootstrap')
...
def create(ctx, jenkins_home, tmp, tar, exclude_plugins, tar_opts, exclude_vcs, ignore_fail, exclude_archive, exclude_target,
           exclude_builds,exclude_build_definitions, exclude_workspace, exclude_maven, exclude_logs, exclude_init_jobs, exclude, dry_run):
    """Create a backup"""
    print("Backing up %s to %s/%s..." %
          (jenkins_home, ctx.obj['BUCKET'], ctx.obj['BUCKET_PREFIX']))
    ...
    if exclude_build_definitions:
        command.append('--exclude=jobs/*/config.xml')        
    ...
    if exclude_init_jobs:
        command.append('--exclude=init.groovy.d/*')
    if exclude_plugins:
        command.append('--exclude=plugins/*')
    for e in exclude:
    ...
    s3 = S3Backups(ctx.obj['BUCKET'],
                   ctx.obj['BUCKET_PREFIX'], ctx.obj['BUCKET_REGION'])
...
def list(ctx):
    ...
    print("All backups for %s/%s..." %
          (ctx.obj['BUCKET'], ctx.obj['BUCKET_PREFIX']))
...
def delete(ctx, backup_id):
    ...
    print("Deleting backup %s in %s/%s..." %
          (backup_id, ctx.obj['BUCKET'], ctx.obj['BUCKET_PREFIX']))
...
def restore(ctx, jenkins_home, tmp, tar, backup_id, tar_opts, dry_run):
    ...
    print("Restoring %s from %s/%s/%s..." %
          (jenkins_home, ctx.obj['BUCKET'], ctx.obj['BUCKET_PREFIX'], backup_id))
